...
let rec eval_expr (expr : Absyn.expr) : float = match expr with
    ...
    | Binary (oper, expr1, expr2) -> 
        try (Hashtbl.find Tables.binary_fn_table oper) 
        (eval_expr expr1) (eval_expr expr2)  
                with Not_found ->   
                       let relop_result = 
                         (Hashtbl.find Tables.relop_table oper)
                         (eval_expr expr1) (eval_expr expr2) in
...
( match memref with 
    |Absyn.Arrayref (v,e) -> Array.set 
    (Hashtbl.find Tables.array_table v)
    (int_of_float (eval_expr e)) (eval_expr expr)
    |Absyn.Variable var -> 
    Hashtbl.add Tables.variable_table var (eval_expr expr)); None
   ...
   (* let input_number memref =*)
        ...
        | first::other -> try let number = Etc.read_number () in
           ((match first with   
            |Absyn.Arrayref (v,e) -> Array.set 
            (Hashtbl.find Tables.array_table v)
            (int_of_float (eval_expr e)) (number)
            |Absyn.Variable var -> 
            Hashtbl.add Tables.variable_table var number);
               (interp_input other))
           with End_of_file -> 
           ( Hashtbl.add Tables.variable_table "eof" 1.;None)
...
let interp_dim (ident : Absyn.ident) (expr : Absyn.expr ) = 
    ...
    (Array.make (int_of_float (eval_expr expr)) 0.); None
